...
На сайте Leap Motion указано [17], что возможна VR интеграция с Unity и Unreal, и, так как для решения поставленной задачи больше подходит среда разработки Unity, в связи с тем, что нет необходимости использовать высококачественные графические эффекты и не требуется высокая вычислительная мощность, выбор был остановлен на данной среде разработки. 

Интеграция Leap Motion и Unity осуществляется при помощи ресурса Leap Motion Core Assets [18]. Он позволяет проекту Unity получать данные из фреймов, в которых Leap Motion API хранит атрибуты для рук, пальцев, костей и суставов пользователя. Эти данные используются в качестве пользовательского ввода.

### Сценарий использования
Чтобы в дальнейшем рассмотреть особенности обработки пользовательского ввода, необходимо определить сценарий использования приложения.
...
Также было разработано приложение для 3D моделирования в VR при помощи hand tracking, которое после запуска не нуждается в пользовательском вводе с мыши/клавиатуры, а управляется исключительно жестами. 

Были предложены решения для некоторых проблем в области интеграции Leap Motion и Unity: процедура перевода координат контроллера в координаты Unity при помощи метода InteractionBox(); упрощение ввода пользователя за счет установления допустимых погрешностей для положения рук и жестов; снижение возможности спутывания жестов используя разделение ввода для левой и правой руки.

Направления дальнейших исследований включают в себя увеличение количества типов взаимодействия с 3D моделью, таких как сплющивание, скручивание, разрывание и т.д. и увеличение количества возможных жестов для пользовательского ввода.
